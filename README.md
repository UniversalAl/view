...
<pre><code>Preview(list of clips or clip[, int[] frames=None, int delay=None, str img_dir=None, str matrix_in_s=None, kernel='Point',
                                 int mod_x=2, int mod_y=2, bool ignore_subsampling = False,
...
Status Bar is present if opencv is compiled with Qt library build though. But at the same time Qt build interfears with arrow key bindings.<br>
